...
- `pnpm typecheck` passes after narrowing the Dialog root `children` prop to `React.ReactNode`; the Base payload-render function no longer leaks into Radix-compatible consumers such as `CommandDialog`.

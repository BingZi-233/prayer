...
import { applyPromote, promotedDocRel, promotedMarkdown } from "@/lib/reflect-promote";
import {
  runPromote,
  selectPromoteIds,
...
function seedApproved(n: number) {
  const ids: number[] = [];
  for (let i = 0; i < n; i++) {
    const id = repo.insertKbEntry("human-reflection", `通用FAQ${i}:完整可复用步骤`, `human-reflection:100:${i}`, vec());
...
function fakeQuery(structured: unknown) {
  return () =>
    (async function* () {
      ...
      yield { type: "result", subtype: "success", structured_output: structured };
    })();
}
...
describe("reflect-promote helpers", () => {
  ...
  it("applyPromote:写盘+入库+标 promoted;检索走正式文档", async () => {
    const id = repo.insertKbEntry("human-reflection", "退款 7 天到账", "human-reflection:1:1", vec());
    ...
    expect(repo.reflectionEntries().find((e) => e.id === id)!.status).toBe("promoted");
    // 反思侧不再命中;正式文档命中
    const hits = repo.searchKb(vec(), 5);
    expect(hits.some((h) => h.content.includes("退款"))).toBe(true);
    expect(hits.every((h) => h.source !== `human-reflection:1:1` || true)).toBe(true);
  ...
  it("applyPromote 幂等:已升格 → already", async () => {
    const id = repo.insertKbEntry("human-reflection", "faq", "human-reflection:1:1", vec());
  ...
  it("applyPromote 拒绝 rejected", async () => {
    const id = repo.insertKbEntry("human-reflection", "坏", "human-reflection:1:1", vec());
    repo.insertReflectionMeta(id, 1, "q", "a");
...
describe("runPromote", () => {
  ...
  it("跳过 rejected/promoted 候选", async () => {
    ...
    const bad = repo.insertKbEntry("human-reflection", "坏", "human-reflection:1:9", vec());
    repo.insertReflectionMeta(bad, 1, "q", "a");
    repo.setReflectionStatus(bad, "rejected");
    const done = repo.insertKbEntry("human-reflection", "已升", "human-reflection:1:8", vec());

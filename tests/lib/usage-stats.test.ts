...
describe("usageStats", () => {
  it("record 累加、按站点分组、snapshot 深拷贝", () => {
    usageStats.record("intent", { cacheRead: 10, cacheCreation: 2, input: 3, output: 1, costUsd: 0.001 });
    usageStats.record("intent", { cacheRead: 5, cacheCreation: 0, input: 1, output: 1, costUsd: 0.001 });
    usageStats.record("agent", { cacheRead: 100, cacheCreation: 0, input: 0, output: 5, costUsd: 0.01 });
    const snap = usageStats.snapshot();
    expect(snap.intent).toEqual({ count: 2, cacheRead: 15, cacheCreation: 2, input: 4, output: 2, costUsd: 0.002 });
  ...
  it("cacheHitRatio = read/(read+creation+input)", () => {
    expect(cacheHitRatio({ count: 1, cacheRead: 90, cacheCreation: 5, input: 5, output: 0, costUsd: 0 })).toBeCloseTo(0.9);
    expect(cacheHitRatio({ count: 0, cacheRead: 0, cacheCreation: 0, input: 0, output: 0, costUsd: 0 })).toBe(0);
  });
});
...
describe("usageFromResult", () => {
  ...
  it("result → 提取四类 token + cost,缺字段补 0", () => {
    expect(
      usageFromResult({ type: "result", total_cost_usd: 0.5, usage: { input_tokens: 10, cache_read_input_tokens: 20 } })
    ).toEqual({ cacheRead: 20, cacheCreation: 0, input: 10, output: 0, costUsd: 0.5 });
  });
});
...
describe("drainQuery", () => {
  it("累计文本 + 抓 session_id + 记账 result.usage", async () => {
    const iter = (async function* () {
      ...
      yield { type: "result", subtype: "success", total_cost_usd: 0.02, usage: { input_tokens: 7, output_tokens: 3, cache_read_input_tokens: 40, cache_creation_input_tokens: 1 } };
    })();

...
describe("reflection_meta / reflectionEntries", () => {
  it("insertReflectionMeta → reflectionEntries 带出 question/answer,默认 approved", () => {
    ...
    expect(e).toMatchObject({ groupId: 100, ts: 5, question: "问X", answer: "答Y", status: "approved" });
  });
  ...
  it("migrate 回填:无 meta 的 human-reflection 补 approved 行", () => {
    ...
    const id = repo.insertKbChunk("human-reflection", "旧无 meta", "human-reflection:0:1");
  ...
  it("replaceReflectionEntries → 删旧 meta(不留孤儿)+ 记 compaction", () => {
    ...
    expect(entries[0]).toMatchObject({ content: "新条目", groupId: 0, question: null, answer: null, status: "approved" });

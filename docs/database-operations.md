...
`pnpm db:report` 将数据库完整性、默认保留策略下的候选行数，以及 outbox 的
`pending`/`sending`/`sent`/`failed` 分布输出为 JSON。它使用只读连接，不运行迁移、
不创建备份，也绝不删除数据库或 transcript：

```bash
pnpm db:report
pnpm db:report ./data/agent.db
```

报告的 `ok: false`（完整性失败或缺少预期表）会以非零状态退出，适合由现有监控系统
触发告警；保留候选数量本身是人工复核信号，不会让命令失败。`failed` outbox 行也可能
仍在重试，需结合运行时和投递详情处理。

可由部署方在低峰期用自己的 cron 或 systemd timer 调用，并将 JSON 保存到受保护的日志
位置，例如：

```cron
15 3 * * * cd /srv/prayer && /path/to/pnpm db:report >> /var/log/prayer/db-report.jsonl 2>&1
```

请把调度作业的非零退出接入现有告警渠道。此报告不代表已经有有效、异地或可恢复的
备份；自动创建备份、保留和恢复演练仍应作为明确授权、可审计的部署作业配置。
...
备份使用 SQLite 在线备份 API，能够包含 WAL 中已提交的数据。源库只用只读句柄打开，创建前检查源数据库，
...
在变更发布前或按部署方的恢复计划，用一份**已生成的备份**运行：

```bash
pnpm db:backup ./data/agent.db ./backups/agent-before-release.db
pnpm db:restore-drill ./backups/agent-before-release.db
```

`db:restore-drill` 必须显式传入备份路径，拒绝把当前 `DB_PATH`、内存库或符号链接作为输入。
它先以只读方式检查备份，再复制到私有临时目录；当前数据库迁移层只会打开该副本，随后
执行完整性、预期保留表与 outbox 状态检查，并在成功返回前删除整个临时目录。输出 JSON
仅包含路径、字节数、schema 版本和运行状态摘要，不包含业务内容。

这个演练不会停止或重启 PM2、不会启动通道、不会写入或替换运行中的数据库，也不会替代异地
备份。它证明“该备份可复制，且能被当前数据库层打开并通过数据库就绪检查”；真实替换
`DB_PATH` 仍必须走下方的停机恢复流程。应把命令输出、备份路径/时间、操作者和结论保存到
部署方受保护的变更记录中。

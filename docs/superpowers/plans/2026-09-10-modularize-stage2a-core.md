...
## Task 3: 测试目录对齐 + 护栏条目去冗余

- [ ] **Step 0: 先做「护栏条目去冗余」**

**为什么**:`RETIRED_PREFIXES` 现在有 14 项,其中 **11 条精确文件项是冗余的**。代码审查查明:

- **忘删死精确规则** → 已被 Task 2 新增的「每条精确文件规则都命中真实存在的文件」断言抓住
- **旧文件复活**(如有人重新建 `lib/bus.ts`)→ 规则已删,`layerOf` 返回 `null`,「lib 下每个文件都归属于某一层」直接抓住

也就是说精确文件项一条都不扛事,只是噪声。而阶段 2b/3/4 要退休的大头**恰恰是精确文件规则**(`lib/channels/types.ts`、`lib/tools/embed.ts`、`lib/agent/*.ts` 等),照现在这样堆下去会涨到 40+ 项。

**目录项要保留** —— 它们扛一个别的检查盖不住的场景:「死目录规则 `lib/db/` 未删 + 有人在该路径下新建文件」时,那条陈旧规则会把这个文件**静默地**归成 `core`,而「每文件归层」因为规则还在所以通过。

改法:把 `RETIRED_PREFIXES` 缩到只留 3 条目录项:

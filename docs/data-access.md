...
- SQLite 事务只覆盖数据库。反思升格流程中的文件写入仍需独立的补偿策略，
  ...
  `docs/kb/retrieval/` 的活跃语料之前，先在 `docs/kb/_meta/` 落 manifest
  （两侧 SHA-256；合并进已有文档时另落 pre-edit 快照，新建文档没有既有
  内容故只有 manifest）；写盘或入库失败时还原文件并把 manifest 改记为
  `rolled_back`。这些 `*.md.disabled` 不属于可入库语料。

...
export class OutboxRepository implements OutboundStore {
  ...
  private row(r: { id:number; delivery_key:string; action_json:string; status:OutboxRecord["status"]; attempts:number; next_attempt_at:number; lease_until:number|null; last_error:string|null }): OutboxRecord { return { id:r.id, deliveryKey:r.delivery_key, action:JSON.parse(r.action_json) as ActionSend, status:r.status, attempts:r.attempts, nextAttemptAt:r.next_attempt_at, leaseUntil:r.lease_until, lastError:r.last_error } }

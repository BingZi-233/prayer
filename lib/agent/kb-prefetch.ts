 ...
 * user prompt,避免「历史里查过就不查了」;kb_search / packy 仍必须可被调用 ——
...
export function formatKbBlock(hits: { content: string }[]): string {
  ...
  return [
    ...
    body,
    "以上只是候选片段,可能过时、不完整或与本轮问题不完全对应。政策/文档类问题:片段不足以覆盖时必须换关键词调用 kb_search 补检。价格/可用模型/分组倍率/公告等实时数据:不要用这些片段里的数字作答,必须调用 packy 取当前值。不要凭记忆或此前轮次的印象推断。",
  ].join("\n")
}

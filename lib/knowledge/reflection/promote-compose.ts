...
/**
 * 提示里的长度预算必须与 unitShapeIssue 的闸门同源(都取
 * DEFAULT_KB_CHUNK_MAX_CHARS):若把数字写死在提示里而常量被改小,模型按旧
 * 预算成文、闸门按新上限拒绝,每一轮升格都会静默全败且看不出原因。
 */
...
- 标题加正文加来源行总长不得超过 ${DEFAULT_KB_CHUNK_MAX_CHARS} 字符。

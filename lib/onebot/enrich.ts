...
export type CallFn = (action: string, params: Record<string, unknown>) => Promise<any>;
...
export async function enrich(parsed: ParsedMessage, deps: EnrichDeps): Promise<IncomingMessage> {
  ...
  if (parsed.replyId) {
    try {
      ...
      if (m) {
        ...
        quoted = [nick, text].filter(Boolean).join(": ") || (imgs.length ? "[图片]" : "");
        imageUrls.push(...imgs);
      }
    } catch {
      /* 回查失败:降级,quoted 留空 */
  ...
  if (parsed.forwardId) {
    try {
      const f = await deps.call("get_forward_msg", { message_id: parsed.forwardId });
      const nodes: any[] = f?.messages ?? f?.message ?? [];
      const parts: string[] = [];
      for (const n of nodes) {
        const { text, imageUrls: imgs } = extractSegments(n?.message ?? n?.content);
        const nick = n?.sender?.nickname ?? n?.sender?.card ?? "";
        parts.push([nick, text || (imgs.length ? "[图片]" : "")].filter(Boolean).join(": "));
        imageUrls.push(...imgs);
      }

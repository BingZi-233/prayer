...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  ...
  let auditApp: ReturnType<typeof getAppContext>
  try {
    auditApp = getAppContext()
  } catch (err) {
    const detail = safeApiError(err)
    logger.error(`[config] 配置应用失败: ${detail}`, {
      scope: "config.reconfigure",
    })
    return NextResponse.json(fail(`配置未生效，未写入：${detail}`), {
      status: 500,
    })
  }
  if (
    parsed.data.dbPath !== undefined &&
    canonicalDbPath(parsed.data.dbPath) !== canonicalDbPath(auditApp.cfg.dbPath)
  )
  ...
  return withAdminMutationAudit(
    ...
    () =>
      serializeRuntimeMutation(async () => {
        ...
        let configRepo:
          ReturnType<typeof getAppContext>["configRepo"] | undefined
        ...
        try {
          ...
          if (
            parsed.data.dbPath !== undefined &&
            canonicalDbPath(parsed.data.dbPath) !==
              canonicalDbPath(current.dbPath)
          ) {
            return NextResponse.json(
              fail("数据库路径由部署环境中的 DB_PATH 管理，不能在线修改"),
              { status: 400 }
          ...
          const next = setConfig(
            configRepo,
            mergeConfigPatch(current, parsed.data)
          )
          persisted = true

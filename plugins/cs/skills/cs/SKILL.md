---
name: cs
description: 检索本仓库客服知识库(产品文档 / FAQ / 人工反思沉淀),回答政策、文档、FAQ、接入、故障排查等事实性问题时,预检索片段不足或没有预检索就必须查。当用户询问产品注册、配置、条款、退款、常见问题,或任何"知识库里怎么说"的问题时使用;价格、库存、版本、状态与公告改走对应的业务工具。底层是本地 bge-small-zh 向量 + sqlite-vec 语义检索,只读,不联网、不改库。
---
...
- 若返回「知识库无相关内容。」→ 知识库确实没有,别硬答。价格、库存、版本、状态与公告这类实时数据,不要用本工具的片段作答,改直接调用对应的业务工具。

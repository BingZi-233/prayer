---
name: cs
description: 检索本仓库客服知识库(产品文档 / FAQ / 人工反思沉淀),回答政策、文档、FAQ、接入、故障排查等事实性问题时,预检索片段不足或没有预检索就必须查。当被问到 PackyAPI 相关的注册、令牌、CLI 接入、ccswitch、绘图、条款、退款、常见问题,或任何"知识库里怎么说"的问题时使用;价格/模型/分组/公告改走 packy。底层是本地 bge-small-zh 向量 + sqlite-vec 语义检索,只读,不联网、不改库。
---
...
- 政策 / 文档 / FAQ / 接入步骤 / 故障排查等事实性问题:消息里的【知识库检索结果】只是预检索候选,不足以覆盖或没有这段时,**必须调 `kb_search`**,再基于返回片段作答;不要凭记忆编造。
- 检索返回**语义最相近的 top-5 片段**(不是精确匹配),按相关度排序,格式 `[1] …\n[2] …`。
- 若返回「知识库无相关内容。」→ 知识库确实没有,别硬答。价格 / 可用模型 ID / 分组倍率 / 高峰价 / 公告这类实时数据,不要用本工具的片段报价,改直接调用 `packy`。

...
`app/`（App Router:`/admin/*` 页面 + `/api/*` 路由）、`lib/`（核心:`agent/` agent+编排+反思循环、`core/db/` better-sqlite3、`channels/` 通道（QQ/TG）、`model/` 模型基座（嵌入/超时/JSON 输出等）、`tools/` KB 检索）、`plugins/`（`cs`/`packyapi` 本地 MCP server）、`scripts/`（ingest）、`docs/kb/`（知识库源,gitignore）、`data/`+`logs/`（gitignore)。
